            ...
            ILogger log)
        {
            ...
            else
            {
                try
                {
                    ...
                    INetworkSecurityGroup networkSecurityGroupInsight = _azureProd.NetworkSecurityGroups.Define($"{clusterName}2-insight-nsg")
                        .WithRegion(region)
                    ...
                    #region Create VM IP
                    IPublicIPAddress publicIpAddress = _azureProd.PublicIPAddresses.Define($"{clusterName}2-insight-ip")
                       .WithRegion(region)
                       .WithExistingResourceGroup(resourceGroupName)
                       .WithDynamicIP()
                       .WithLeafDomainLabel($"{clusterName}2-insight-ip")
                       .WithTag("_contact_person", contactPerson)
                       .Create();
                    ...
                    INetworkInterface networkInterfaceInsight = _azureProd.NetworkInterfaces.Define($"{clusterName}2-insight-nic")
                        .WithRegion(region)
                        .WithExistingResourceGroup(resourceGroup)
                    ...
                    IVirtualMachine createVmInsight = _azureProd.VirtualMachines.Define($"{clusterName}2-insight")
                        .WithRegion(region)
                        .WithExistingResourceGroup(resourceGroup)
